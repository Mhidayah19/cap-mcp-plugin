...
describe("Subscriber XSUAA URL Resolution", () => {
  describe("extractSubscriberSubdomain", () => {
    it("should extract subdomain from subscriber approuter URL", () => {
      // Subscriber URL: xba-dev-eu10.dev.mymediset.cloud
      const result = extractSubscriberSubdomain(
        "xba-dev-eu10.dev.mymediset.cloud",
      );

      expect(result.subdomain).toBe("xba-dev-eu10");
    ...
    it("should strip app prefix from subscriber subdomain", () => {
      // URL pattern: mymediset-xba-dev-eu10.dev.mymediset.cloud
      // Expected: xba-dev-eu10 (strip "mymediset-" prefix)
      const result = extractSubscriberSubdomain(
        "mymediset-xba-dev-eu10.dev.mymediset.cloud",
        { appName: "mymediset" },
      );
    ...
    it("should NOT return provider subdomain for subscriber request", () => {
      ...
      const providerSubdomain = "provider-dev-5dbg7fzn";
    ...
    it("should use fallback subdomain for local development", () => {
      ...
      const result = extractSubscriberSubdomain("localhost:4004", {
        fallbackSubdomain: "provider-dev-5dbg7fzn",
      });

      expect(result.subdomain).toBe("provider-dev-5dbg7fzn");
    ...
    it("should not strip prefix when app name does not match", () => {
      ...
      const result = extractSubscriberSubdomain(
        "otherapp-xba-dev-eu10.dev.mymediset.cloud",
        { appName: "mymediset" },
      );

      expect(result.subdomain).toBe("otherapp-xba-dev-eu10");
      expect(result.wasStripped).toBe(false);
    });
  ...
  describe("buildSubscriberXsuaaTokenUrl", () => {
    ...
    it("should build correct subscriber XSUAA token URL", () => {
      ...
      expect(url).toBe(
        "https://xba-dev-eu10.authentication.eu10.hana.ondemand.com/oauth/token",
      );
    });
    ...
    it("should NOT build provider XSUAA URL for subscriber subdomain", () => {
      ...
      expect(url).not.toContain("provider-dev-5dbg7fzn");
    });
  ...
  describe("Multi-Tenant Token URL Resolution (Integration)", () => {
    ...
    it("should build subscriber token URL from approuter host", () => {
      // Simulate request from subscriber: mymediset-xba-dev-eu10.dev.mymediset.cloud
      const effectiveHost = "mymediset-xba-dev-eu10.dev.mymediset.cloud";
      const appName = "mymediset";
    ...
    it("should preserve subscriber context through token exchange", () => {
      ...
      const subscriberZid = "2a6e99f2-5dbf-4b82-809e-e53b510f8a88";
      const subscriberZdn = "xba-dev-eu10";
      const providerZdn = "provider-dev-5dbg7fzn";
